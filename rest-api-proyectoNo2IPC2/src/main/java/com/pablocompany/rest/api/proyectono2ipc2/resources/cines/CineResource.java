...
import com.pablocompany.rest.api.proyectono2ipc2.cine.dtos.CineResponse;
...
public class CineResource {
    ...
    public Response obtenerCine(
        ...
        try {
            ...
            Cine cineEditar = cineCrudService.obtenerCinePorCodigo(codigoCine);

            return Response.ok(new CineResponse(cineEditar)).build();
    ...
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response editarCine(EditarCineRequest request) {

        CineCrudService cineCrudService = new CineCrudService();
        try {

            if (cineCrudService.editarCine(request)) {
                return Response.ok().build();
            } else {
                throw new ErrorInesperadoException("No se ha podido crear el nuevo cine");
            }
