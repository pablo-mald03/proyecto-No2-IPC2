...
public class ReporteBoletosVendidosDB {
    ...
    //Constante que permite obtener el reporte de boletos comprados en TODO INTERVALO DE TIEMPO SIN FILTRO
    private final String REPORTE_TODO_BOLETOS = "SELECT sa.codigo, ci.nombre AS `cineAsociado`, sa.nombre, sa.ubicacion, COUNT(pb.numero) AS `totalBoletosVendidos`, SUM(pb.monto) AS `totalRecaudado` FROM sala AS `sa` JOIN cine AS `ci` ON sa.codigo_cine = ci.codigo JOIN pago_boleto AS `pb` ON sa.codigo = pb.codigo_sala GROUP BY sa.codigo, ci.nombre, sa.nombre, sa.filas, sa.columnas, sa.ubicacion ORDER BY totalRecaudado DESC LIMIT ? OFFSET ?";

    //Constante que permite obtener el conteo de cantidad de reportes que hay EN TODO INTERVALO DE TIEMPO
    private final String CANTIDAD_TODO_REPORTES = "SELECT COUNT(*) AS cantidad FROM ( SELECT sa.codigo FROM sala AS sa JOIN cine AS ci ON sa.codigo_cine = ci.codigo JOIN pago_boleto AS pb ON sa.codigo = pb.codigo_sala GROUP BY sa.codigo, ci.nombre, sa.nombre, sa.filas, sa.columnas, sa.ubicacion ) AS conteo";

    //Constante que permite obtener el conteo de cantidad de reportes que hay EN TODO INTERVALO DE TIEMPO con filtro por sala de cine
    private final String CANTIDAD_REPORTES_TODO_FILTRO = "SELECT COUNT(*) AS cantidad FROM ( SELECT sa.codigo FROM sala AS sa JOIN cine AS ci ON sa.codigo_cine = ci.codigo JOIN pago_boleto AS pb ON sa.codigo = pb.codigo_sala WHERE sa.codigo = ? GROUP BY sa.codigo, ci.nombre, sa.nombre, sa.filas, sa.columnas, sa.ubicacion ) AS conteo";

    //Constante que permite obtener el reporte de los boletos que ha comprado el usuario EN TODO INTERVALO DE TIEMPO
    private final String USUARIOS_TODO_SALA = "SELECT u.id AS `idUsuario`, u.nombre, COUNT(pb.numero) AS `boletosComprados`, SUM(pb.monto) AS `totalPagado`  FROM sala AS `sa` JOIN pago_boleto AS `pb` ON sa.codigo = pb.codigo_sala JOIN usuario AS `u` ON pb.id_usuario = u.id WHERE sa.codigo =  ? GROUP  BY  u.id, u.nombre ORDER BY totalPagado DESC";

    //Constante que permite obtener el registro de pagos de boleto en TODO intervalo de tiempo CON FILTRO
    private final String REPORTE_BOLETOS_TODO_FILTRO = "SELECT sa.codigo, ci.nombre AS `cineAsociado`, sa.nombre, sa.ubicacion, COUNT(pb.numero) AS `totalBoletosVendidos`, SUM(pb.monto) AS `totalRecaudado` FROM sala AS `sa` JOIN cine AS `ci` ON sa.codigo_cine = ci.codigo JOIN pago_boleto AS `pb` ON sa.codigo = pb.codigo_sala WHERE sa.codigo = ? GROUP BY sa.codigo, ci.nombre, sa.nombre, sa.filas, sa.columnas, sa.ubicacion ORDER BY totalRecaudado DESC LIMIT ? OFFSET ?";

    //Metodo delegado para obtener la cantidad de reportes de boletos vendidos que se tienen en el intervalo de fechas
    ...
    //Metodo delegado para obtener la cantidad de reportes de boletos vendidos que se tienen en el intervalo de fechas con filtro
    ...
    //Metodo que sirve para poder consultar el reporte de boletos vendidos SIN FLITRO
    ...
    //Metodo que sirve para poder consultar el reporte de boletos vendidos CON FLITRO
    ...
    //Metodo delegado para obtener la cantidad de boletos de cine en todo intervalo de tiempo
    ...
    //Metodo delegado para obtener la cantidad de reportes de boletos vendidos EN TODO INTERVALO DE TIEMPO con filtro
    public int cantidadReportesTodoFiltro(ReporteRequest reporteSalas) throws ErrorInesperadoException, DatosNoEncontradosException {
    ...
    //Metodo que sirve para poder consultar el reporte boletos vendidos EN TODO INTERVALO DE TIEMPO SIN FLITRO
    public List<ReporteBoletosVendidosDTO> obtenerReporteTodoBoletodVendidos(ReporteRequest reporteSalas) throws ErrorInesperadoException, FormatoInvalidoException {
        ...
        try (PreparedStatement query = connection.prepareStatement(REPORTE_TODO_BOLETOS);) {
            ...
            for (ReporteBoletosVendidosDTO reporte : listadoReportes) {
                reporte.setUsuarios(obtenerTodoBoletos(reporteSalas, reporte.getCodigo()));
            }
    ...
    //Metodo que sirve para poder consultar el reportes de boletos vendidos EN TODO INTERVALO DE TIEMPO CON FLITRO
    public List<ReporteBoletosVendidosDTO> obtenerReporteTodoBoletosVendidosFiltro(ReporteRequest reporteSalas) throws ErrorInesperadoException, FormatoInvalidoException {
        ...
        Connection connection = DBConnectionSingleton.getInstance().getConnection();
        try (PreparedStatement query = connection.prepareStatement(REPORTE_BOLETOS_TODO_FILTRO);) {
    ...
    //Metodo que sirve para obtener las valificaciones que hacen los usuarios en las salas
    private List< UsuarioBoletosCompradosDTO> obtenerTodoBoletos(ReporteRequest reporteSalas, String idSala) throws FormatoInvalidoException, ErrorInesperadoException {
