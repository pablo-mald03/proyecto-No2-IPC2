...
public class ReporteSalaCineDB {
    ...
    //Constante que permite obtener el reporte de comentarios sin filtro de sala EN CUALQUIER INTERVALO
    private final String REPORTE_TODO_COMENTARIOS = "SELECT sa.codigo, ci.nombre AS `cineAsociado`, sa.nombre, sa.filas, sa.columnas, sa.ubicacion FROM sala AS `sa` JOIN cine AS `ci` ON sa.codigo_cine = ci.codigo JOIN comentario_sala AS `co` ON sa.codigo = co.codigo_sala ORDER BY co.fecha_posteo DESC LIMIT ? OFFSET ?";

    //Constante que permite obtener el conteo de cantidad de reportes que hay en base a una fecha EN CUALQUIER INTERVALO
    private final String CANTIDAD_TODO_REPORTES = "SELECT COUNT(*) AS `cantidad` FROM sala AS `sa` JOIN comentario_sala AS `co` ON sa.codigo = co.codigo_sala";

    //Constante que permite obtener el conteo de cantidad de reportes que hay en base a una fecha con filtro por sala de cine EN CUALQUIER INTERVALO
    private final String CANTIDAD_REPORTES_TODO_FILTRO = "SELECT COUNT(*) AS `cantidad` FROM sala AS `sa` JOIN comentario_sala AS `co` ON sa.codigo = co.codigo_sala WHERE sa.codigo = ? ";

    //Constante que permite obtener el preorte de comentarios en base al filtro de sala EN CUALQUIER INTERVALO
    private final String COMENTARIOS_TODO_SALAS = "SELECT co.id_usuario, co.contenido, co.fecha_posteo FROM sala AS `sa` JOIN comentario_sala AS `co` ON sa.codigo = co.codigo_sala WHERE sa.codigo = ? ORDER BY co.fecha_posteo DESC";

    //Constante que permite obtener los comentarios de una sala de cine con filtro de sala de cine EN CUALQUIER INTERVALO
    private final String REPORTE_COMENTARIOS_TODO_FILTRO = "SELECT sa.codigo, ci.nombre AS `cineAsociado`, sa.nombre, sa.filas, sa.columnas, sa.ubicacion FROM sala AS `sa` JOIN cine AS `ci` ON sa.codigo_cine = ci.codigo JOIN comentario_sala AS `co` ON sa.codigo = co.codigo_sala WHERE sa.codigo = ? ORDER BY co.fecha_posteo DESC LIMIT ? OFFSET ?";
    ...
    public List<ReporteSalasComentadasDTO> obtenerReporteTodoComentarios(ReporteRequest reporteSalas) throws ErrorInesperadoException, FormatoInvalidoException {
        ...
        try (PreparedStatement query = connection.prepareStatement(REPORTE_TODO_COMENTARIOS);) {
    ...
    //Metodo que sirve para poder consultar el reporte de comentarios SIN FLITRO
    public List<ReporteSalasComentadasDTO> obtenerReporteTodoComentariosFiltro(ReporteRequest reporteSalas) throws ErrorInesperadoException, FormatoInvalidoException {
        ...
        Connection connection = DBConnectionSingleton.getInstance().getConnection();
        try (PreparedStatement query = connection.prepareStatement(REPORTE_COMENTARIOS_TODO_FILTRO);) {
